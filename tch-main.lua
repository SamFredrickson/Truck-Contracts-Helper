...
function main()
    if not isSampLoaded() or not isSampfuncsLoaded() then return end
		...
		sampAddChatMessage(
			"{FFFFFF}ћеню настроек - {ed5a5a}/tch.menu{FFFFFF}, страница скрипта: {ed5a5a}" .. 
			...
			function()
				if not httpService.version then
					local localMessage = LocalMessage.new(
						"{FFFFFF}ѕроизошла {ed5a5a}ошибка {FFFFFF}при попытке обновлени€. " ..
						"—в€житесь с разработчиком скрипта."
					)
				...
				if httpService.version.number == constants.SCRIPT_INFO.VERSION_NUMBER then
					local localMessage = LocalMessage.new(
						"{FFFFFF}” вас уже установлена {ed5a5a}" .. 
						"актуальна€ {FFFFFF}верси€ скрипта."
					)
					chatService.send(localMessage)
					return
				end
				...
				lua_thread.create(function()
					local messages = {
						LocalMessage.new(
							"{FFFFFF}Ќе забудьте распаковать {ed5a5a}архив {FFFFFF}в папке " .. 
							"{ed5a5a}moonloader {FFFFFF}с заменой старых файлов." 
						),
						LocalMessage.new(
							"{FFFFFF}ѕереход по ссылке дл€ скачивани€ через {ed5a5a}3 секунды..." 
						)
					}
					local commands = {
						string.format(
		...
		sampRegisterChatCommand(
			...
			function(args) 
				if args == nil or args == "" then
					local localMessage = LocalMessage.new("{ed5a5a}/tch.coords.send{FFFFFF} [текст сообщени€]")
					chatService.send(localMessage)
					return
				end
...
function sampev.onServerMessage(color, text)
	...
	if text:find(serverMessageService.findByCode("has-active-contract").message) then
		...
		local localMessage = LocalMessage.new(
			"{FFFFFF}” вас уже есть {ed5a5a}активный {FFFFFF}контракт."
		)
